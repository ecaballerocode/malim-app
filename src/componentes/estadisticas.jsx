...
console.log("Hola")
  ...
  const ingresos = () => {
    // Extraer los pagos de los pedidos que tengan la propiedad "pagos"
    const arregloPagos = Object.values(pedidos)
      .filter((pedido) => Array.isArray(pedido.pagos)) // Verificar que "pagos" sea un arreglo
      .flatMap((pedido) => pedido.pagos); // Unir todos los pagos en un solo arreglo
  
    // Filtrar los pagos dentro del rango de fechas
    const filtrarPagos = () => {
      return arregloPagos.filter((pago) => {
        const pedidoFecha = new Date(pago.fecha);
        const inicio = fechaInicio ? new Date(fechaInicio) : null;
        const fin = fechaFin ? new Date(fechaFin) : null;
          ...
          <div className="bg-pink-100 z-0 mx-5 mt-2 p-2 h-64 shadow-xl text-center rounded-lg border-pink-400 border-2">
            ...
            <div className="grid grid-cols-2 gap-4 m-5">
              ...
              <div className="mt-5">
                ...
                <p className="text-pink-800 text-lg mt-1 font-bold">%{Math.floor(((ingresos() - inversionPeriodo()) / ingresos()) * 100)}</p>
              </div>
            </div>
          </div>
